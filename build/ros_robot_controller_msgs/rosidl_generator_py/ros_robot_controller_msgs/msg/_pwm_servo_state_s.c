...
bool ros_robot_controller_msgs__msg__pwm_servo_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("ros_robot_controller_msgs.msg._pwm_servo_state.PWMServoState", full_classname_dest, 60) == 0);
  }
  ros_robot_controller_msgs__msg__PWMServoState * ros_message = _ros_message;
  {  // id
  ...
  {  // position
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      uint16_t * dest = ros_message->position.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
  ...
  {  // offset
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      int16_t * dest = ros_message->offset.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
PyObject * ros_robot_controller_msgs__msg__pwm_servo_state__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("ros_robot_controller_msgs.msg._pwm_servo_state");
  ...
  {  // position
    ...
    if (ros_message->position.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      uint16_t * src = &(ros_message->position.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->position.size * sizeof(uint16_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
  ...
  {  // offset
    ...
    if (ros_message->offset.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int16_t * src = &(ros_message->offset.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->offset.size * sizeof(int16_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
